...
class DriverList extends AbstractList {
	...
	public function __construct( Action $filter, Action $new, Action $edit, Action $active, Action $search, Action $transfer, Action $password, array $optResultCenter, array $showAgencies = null) {
		...
		$input = new CheckBoxList('categories', true);
		...
		$table->buildColumnText('driverLicense', 'CNH', null, 80, null, function (DriverLicense $value) {
			return implode('', $value->getCategories());
		});
		$table->buildColumnText('d.active', 'Status', clone $filter, 70, null, function ( $value, User $obj  ) {
			return $obj->getDriverLicense()->getActive() ? new Label('Ativo', Label::Success) : new Label('Inativo', Label::Important);
		});
		if ($showAgencies) {
		...
		$table->buildColumnAction('active', new Icon('icon-remove'), $active, null, function( Button $button, User $obj ) {
			$button->setIcon(new Icon($obj->getDriverLicense()->getActive() ? 'icon-remove' : 'icon-ok'));
		});
