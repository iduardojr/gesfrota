...
class ProductCategoryList extends AbstractList {
	...
	public function __construct( Action $filter, Action $new, Action $edit, Action $active ) {
		$panel = $this->buildPanel('Banco de Especificações', 'Gerenciar Categorias');
		...
		$input = new CheckBox('only-active', 'Apenas categorias ativas');
		...
		$table = $this->buildTable('product-category-list');
		...
		$table->buildColumnAction('active', new Icon('icon-remove'), $active, null, function( Button $button, ProductCategory $category ) {
			...
			if ( $category->getParent() ) {
				$button->setDisabled(! $category->getParent()->getActive());
			}
		});
		$table->buildColumnAction('new', new Icon('icon-plus-sign'), clone $new);
	}
